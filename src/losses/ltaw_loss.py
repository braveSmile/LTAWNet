...
class LTAWLoss(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config  # config under the global namespace
        self.loss_config = config['ltaw']['loss']
        self.match_type = self.config['ltaw']['match_coarse']['match_type']
        self.sparse_spvs = self.config['ltaw']['match_coarse']['sparse_spvs']
...
class NestedTensor(object):
    ...
    def forward(self,inputs: NestedTensor, data):
        """
        Update:
            data (dict): update{
        ...
        # 3. disparity loss
        loss_disp = self.compute_l1_loss(outputs['disp_pred'], inputs, invalid_mask)
        loss_occ = self.compute_entropy_loss(outputs['occ_pred'], inputs, invalid_mask)
        loss = loss_disp * self.loss_config['disp_weight']
        loss_scalars.update({"loss_disp": loss_disp.clone().detach().cpu()})