...
from src.ltaw import LTAW, default_cfg
...
if __name__ == '__main__':
    ...
    # Initialize LTAW
    matcher = LTAW(config=default_cfg)
    matcher.load_state_dict(torch.load(opt.weight)['state_dict'])
    matcher = matcher.eval().to(device=device)

    # Configure I/O
    if opt.save_video:
        print('Writing video to ltaw-matches.mp4...')
        writer = cv2.VideoWriter('ltaw-matches.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 15, (640*2 + 10, 480))
    if opt.save_input:
